...
- **Role-Based Access Control**: Granular permission system with distinct user and admin roles to
  ensure secure access to platform features.
- **Account Management**: Comprehensive account lifecycle management including deactivation,
  reactivation requests, and handling of inactive accounts.
- **Admin Dashboard**: Powerful administrative tools for user management, content moderation, and
  platform analytics.
- **API Documentation**: Comprehensive OpenAPI documentation for all endpoints, facilitating
...
> 2. Populate the `.env` file (in project root folder):
     >
     >
* Replace the placeholder values in the `.env` file with your actual database credentials and Spring
  Datasource configuration details for both the `pixshare_db` and `pixshare_test_db` databases.
>
...
>    ```
     >
     >    **Note:** Ensure the `.env` file (located in the project root) contains all necessary
     database credentials (username, password) and Spring Datasource configuration for *pixshare_db*
     and *pixshare_test_db* before running the above Docker Compose command to avoid errors.
...
> 1. Populate the `.env` file (in project root folder):
     >
     >
* Fill in the remaining placeholders in the `.env` file with the values specific to your desired
  configuration.
>
...
> 2. Populate the `.env` file (in frontend folder):
     >
     >
* In the `.env` file, replace placeholders with your backend API configuration and other relevant
...
> To use the application, open your browser and access the frontend interface at
`http://localhost:5173`. Ensure the backend server is running at `http://localhost:8081` to enable
> API interactions.
